/* ============================================================
   ...
   panels. Reads SNAP, ASIA_AVG, COMP, SERIES from data.js.
   ============================================================ */
...
(function(){
  ...
  E('text',{x:bx(ASIA_AVG)+6,y:MT+SNAP.length*rowH+18,class:'bar-note',fill:'#C9A24B'},s).textContent='Asian bank average · 1.6% (ADB)';
  SNAP.forEach((r,i)=>{
    const y=MT+i*rowH;
    E('text',{x:ML-12,y:y+barH/2+4,'text-anchor':'end',class:'bar-lab','font-weight':r.bd?600:400,fill:r.bd?'#E8434F':'#CBD3BC'},s).textContent=r.c;
    E('rect',{x:ML,y:y,width:0,height:barH,rx:2,fill:r.bd?'#E8434F':'#E2B33C',opacity:r.bd?1:.6,class:'snap-bar','data-w':bx(r.v)-ML},s);
    E('text',{x:bx(r.v)+10,y:y+barH/2+4,class:'bar-val',fill:r.bd?'#E8434F':'#F0E8D2'},s).textContent=r.v+'%';
    if(r.note)E('text',{x:bx(r.v)+70,y:y+barH/2+4,class:'bar-note'},s).textContent=r.note;
  });
...
(function(){
  ...
  COMP.forEach(cn=>{
    ...
    panel.innerHTML=`<h4>${cn.name}<span class="now">now ${cn.now}</span></h4><div class="storyl">${cn.story}</div>`;
    ...
    [0,25,50].forEach(v=>{
      ...
      mk('text',{x:ml-5,y:py_(v)+3,'text-anchor':'end',class:'mini-axis'}).textContent=v;
    });
    [2000,2010,2020].forEach(yr=>mk('text',{x:px_(yr),y:PH-8,'text-anchor':'middle',class:'mini-axis'}).textContent=yr);
    let db='';
    ...
    if(cn.note){
      ...
      const tx=cn.note.x>2016?nx-8:nx+8;
      mk('circle',{cx:nx,cy:ny,r:4.2,fill:'none',stroke:'#8FB6E8','stroke-width':1.6});
      mk('text',{x:tx,y:ny-8,'text-anchor':anchor,class:'mini-note'}).textContent=cn.note.t;
    }
    panel.appendChild(ps);
    const take=document.createElement('p');
    take.className='take';take.textContent=cn.take;
